...
//==============================================================================
//texture Variables
//static int Texture wallTexture;
//static Texture floorTexture; 
//GLuint loadBMP_custom(const char * imagepath);
//GLuint image = loadBMP_custom("1.bmp");
...
struct Player
{
    ...
    int dx,dz;
    bool isGo;
}player,ex,badguy1,badguy2,badguy3,badguy4,food1,food2,food3,food4;
...
void animate()
{
    ...
    /*
    if ((player.x == food1.x)&&(player.z==food1.z))								//Work in progress to fix bug
    {
    	if(food1_count<2)
    	{
    		j=1;
...
void key( unsigned char key, int x, int y)
{
    ...
    if ((key=='z' || key=='Z') && (distance<58.) ) distance+=0.25;  // Zoom Camera (+)
    if ((key=='x' || key=='X') && (distance>3.) ) distance-=0.25;   // zoom Camera (-)
...
int main (int argc, char** argv)
{
    ...
    glWin = glutCreateWindow("The Maze Game V 0.2.5");
    init();
